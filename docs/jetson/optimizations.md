...
Jetson might come with `8GB.swap`, if so, this needs to be replaced with `16GB.swap`.
...
jetson@yahboom:/$ sudo fallocate -l 16G /var/16GB.swap
jetson@yahboom:/$ sudo chmod 600 /var/16GB.swap
jetson@yahboom:/$ sudo mkswap /var/16GB.swap
Setting up swapspace version 1, size = 16 GiB (17179865088 bytes)
no label, UUID=1279a20a-74ca-492f-a912-eeda0eee8f1e
jetson@yahboom:/$ sudo swapon /var/16GB.swap
```

Make this change persist by adding the following line to `/etc/fstab` and comment out lines referring to `/var/8GB.swap`:  
```bash
/var/16GB.swap none swap sw 0 0
```
